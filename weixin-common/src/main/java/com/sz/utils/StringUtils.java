...
public class StringUtils {
	...
	public static String creatRandomImageName(String extendName) {
		...
		String extmyFileName = extendName.substring(extendName.lastIndexOf(".")) ;
		String randomImageName = "mximgs" + sdate + number + extmyFileName;
		return randomImageName;
	}
	
	public static String creatRandomFilesName(String extendName) {
		Date date = new Date();
		SimpleDateFormat format = new SimpleDateFormat("yyyyMMddkkmmss");
		String sdate = format.format(date);
		Random random = new Random();
		int number = random.nextInt(999999999);
		String extmyFileName = extendName.substring(extendName.lastIndexOf(".")) ;
		String randomImageName = "mxfiles" + sdate + number + extmyFileName;
		return randomImageName;
	}
	...
	/**  生成序列号 */  
	public static final String getSeqFive(int coverPad) {  
		for (;;) {  
			int current = atomic.get();  
	        int newValue = current >= maxvaluefive ? minvaluefive : current + 1;  
	        if (atomic.compareAndSet(current, newValue)) {  
	        	return org.apache.commons.lang3.StringUtils.leftPad(String.valueOf(current), coverPad, "0");  
	        }  
	    }  
	}
	 ...
	 * @return
	 */
	public static boolean isRightCarNum(String carNum) {
